...
namespace Movies
{
    ...
    public static class MovieDatabase
    {
        ...
        public static IEnumerable<Movie> FilterByGenres(IEnumerable<Movie> movies, IEnumerable<string> genres) {
            if (genres == null) return movies;
            List<Movie> results = new List<Movie>();
            foreach (Movie movie in movies) {
                if (genres.Contains(movie.MajorGenre)) {
                    results.Add(movie);
                }
            }
            return results;
        ...
        /// <param name="max">The maximum range value</param>
        /// <returns>The filtered movie collection</returns>
        public static IEnumerable<Movie> FilterByTomRating(IEnumerable<Movie> movies, int? min, int? max) {
            if (min == null && max == null) return movies;
            var results = new List<Movie>();
            if (min == null) {
                foreach (Movie movie in movies) {
